...
disch_full_file = get_discharge_summaries.write_discharge_summaries(out_file="%s/disch_full_multigram.csv" % MIMIC_3_DIR)
...
df = pd.read_csv('%s/disch_full_multigram.csv' % MIMIC_3_DIR)
...
sorted_file = '%s/disch_full_multigram.csv' % MIMIC_3_DIR
df.to_csv(sorted_file, index=False)


# In[ ]:
out_filename = "notes_labeled_multigram"
labeled = concat_and_split.concat_data('%s/ALL_CODES_filtered.csv' % MIMIC_3_DIR, sorted_file, out_filename)
...
fname = '%s/notes_labeled_multigram.csv' % MIMIC_3_DIR
base_name = "%s/multi_disch" % MIMIC_3_DIR #for output
...
vocab_min = 2
vname = '%s/vocab_multi.csv' % MIMIC_3_DIR
...
for splt in ['train', 'dev', 'test']:
    filename = '%s/multi_disch_%s_split.csv' % (MIMIC_3_DIR, splt)
    df = pd.read_csv(filename)
    df['length'] = df.apply(lambda row: len(str(row['TEXT']).split()), axis=1)
    df = df.sort_values(['length'])
    df.to_csv('%s/multi_%s_full.csv' % (MIMIC_3_DIR, splt), index=False)
...
w2v_file = word_embeddings.word_embeddings('full_multigram', '%s/disch_full_multigram.csv' % MIMIC_3_DIR, 100, 0, 5)
...
extract_wvs.gensim_to_embeddings('%s/processed_full_multigram.w2v' % MIMIC_3_DIR, '%s/vocab_multi.csv' % MIMIC_3_DIR, Y)
